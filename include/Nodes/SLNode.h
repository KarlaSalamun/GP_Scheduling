...
class SLNode : public AbstractNode {
    public:
        ...
        double calculate_priority( Task *&task, std::vector<Task *> , std::vector<Task *>, size_t time ) {
            return std::max( task->get_abs_due_date() - task->get_duration() - time, 0. );
        }

        void execute( void *ctx )
        {
            struct task_ctx *ctx_ = reinterpret_cast<struct task_ctx *>(ctx);
            ctx_->task->set_priority( std::max( ctx_->task->get_abs_due_date() - ctx_->task->get_duration() - ctx_->time, 0. ) );
        }
};
